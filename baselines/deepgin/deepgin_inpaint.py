...
from gin_loader import get_loader
...
if not os.path.exists('baselines/deepgin/results/'):
            os.makedirs('baselines/deepgin/results/')
    
if not os.path.exists('baselines/deepgin/imgs/'):
        os.makedirs('baselines/deepgin/imgs/')

if not os.path.exists('baselines/deepgin/masks/'):
        os.makedirs('baselines/deepgin/masks/')

coco_test_loader = get_loader(root='datasets/coco/val2017', \
                                        json='datasets/coco/annotations/instances_val2017.json', \
                                            shuffle=True)

dataset_size = len(coco_test_loader) 
...
for i, data in tqdm(enumerate(coco_test_loader, 0)):
    ...
    print(bar, end='\r')
    with torch.no_grad(): 
        oimgs, omsks, paths = data 
        
        omsk = omsks[0]
        oimg = oimgs[0]
        path = paths[0]
        
        oimg = oimg.permute(1, 2, 0).numpy()

        omsk = omsk.squeeze(0).numpy() * 255.

        oimg = oimg.astype('uint8')
        omsk = omsk.astype('uint8')
